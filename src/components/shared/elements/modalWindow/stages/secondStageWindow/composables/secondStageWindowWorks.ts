...
import type { Stage } from "../SecondStageWindow.vue";

export default function secondStageWindowWorks (props: { data: Stage }, emit: (e: 'close') => void) {  
  ...
  const original = computed(() => localStage.grades.find(item => item.criteria === 'Original')?.grades ?? []);
